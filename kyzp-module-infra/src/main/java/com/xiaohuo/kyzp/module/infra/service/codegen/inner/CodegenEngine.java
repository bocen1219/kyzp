package com.xiaohuo.kyzp.module.infra.service.codegen.inner;
...
import com.xiaohuo.kyzp.framework.apilog.core.annotation.ApiAccessLog;
import com.xiaohuo.kyzp.framework.apilog.core.enums.OperateTypeEnum;
...
import com.xiaohuo.kyzp.framework.common.pojo.PageParam;
import com.xiaohuo.kyzp.framework.common.pojo.PageResult;
import com.xiaohuo.kyzp.framework.common.util.collection.CollectionUtils;
import com.xiaohuo.kyzp.framework.common.util.date.DateUtils;
import com.xiaohuo.kyzp.framework.common.util.date.LocalDateTimeUtils;
import com.xiaohuo.kyzp.framework.common.util.object.BeanUtils;
import com.xiaohuo.kyzp.framework.common.util.object.ObjectUtils;
import com.xiaohuo.kyzp.framework.common.util.string.StrUtils;
import com.xiaohuo.kyzp.framework.excel.core.annotations.DictFormat;
import com.xiaohuo.kyzp.framework.excel.core.convert.DictConvert;
import com.xiaohuo.kyzp.framework.excel.core.util.ExcelUtils;
import com.xiaohuo.kyzp.framework.mybatis.core.dataobject.BaseDO;
...
import com.xiaohuo.kyzp.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.xiaohuo.kyzp.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.xiaohuo.kyzp.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.xiaohuo.kyzp.module.infra.enums.codegen.CodegenSceneEnum;
import com.xiaohuo.kyzp.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.xiaohuo.kyzp.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.xiaohuo.kyzp.module.infra.framework.codegen.config.CodegenProperties;
...
import static com.xiaohuo.kyzp.framework.common.exception.util.ServiceExceptionUtil.exception;
import static com.xiaohuo.kyzp.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_FIELD_DUPLICATE;
import static com.xiaohuo.kyzp.module.infra.enums.ErrorCodeConstants.CODEGEN_MASTER_TABLE_NAME_DUPLICATE;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 kyzp-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 kyzp-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.xiaohuo.kyzp.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/kyzp-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/kyzp-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "kyzp-module-${table.moduleName}/" + // 顶级模块
                "kyzp-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "kyzp-module-${table.moduleName}/" + // 顶级模块
                "kyzp-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "kyzp-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "kyzp-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "kyzp-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "kyzp-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
